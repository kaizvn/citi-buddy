...
## Introduction
  Since I cannot handle all the ideas in my brain cell, I write them down here.
...
## Techstack and Tools
  ...
  - [Next.JS](https://nextjs.org): "Framework to develop Application with React UI", help to handle routing.
  ...
  *Fun fact: I have not used v0, Prisma ever before. And only experienced TailwindCSS, Next.JS and Playwright older version. This challenge is a good occasion for me to self-review my learning ability.*


## Approach
The challenge scope is open with a lot of uncertainty, so that Bottom-Up is a feasible approach in this case. V0 is a game-changer to support me to do UI-First development.
  

## Roadmap
    
- [x] Targeting to deliver a POC that manages Public Utilities of the City: create new data manually input or by streaming *.csv file upload and stream files via URL.
...
## Problems
Come across the requirements, and after having an overview the big picture while developing the UI to input data, I acknowledge main problems that need to be solved:
- DataLog of each Utility which are imported from other sources in different forms, which we may not standardize or take the effort to maintain new DataLog structures.
...
A City may have many Utility services, and each service has different DataLog's forms from different sources (stream, upload, input manually). And, because data is power, so we should collect them all.
...
To support diversity of raw data, I define two types of attributes: Primary data field and Metadata.
 ...
 - **Metadata**: specific attributes of each Utility. Should be updated and weighted by popularity.
  
The minimum validation here is Primary Attributes, which enough to create a record of amount utility services to be logged; metadata is extra info that will be stored in a pair of tables: **Metadata** and **MetadataValue**.

Basically, the relationship of objects should be: 
 - City has many Utility services.
 - Each services has its own metadata, which are grown by variety of import logs.
 - Fields are not Primary Attributes will be checked if existing in the database yet, if not they will be treated as a Metadata and stored in Metadata table.
...
Dashboard
 - UI supports basic tracking volume of Utilities.
 - Displaying data of 10 latest days in two modes: chart or list.
...
## Room for improvements
  ...
  - The current sourcecode is lacks of APIs that support analysis & improve the usability of importing DataLog:

    - Get and map Metadata into DataLog.
    - Function that maps utility's name to utility_id.
    - Function that maps City to city_id.
  
  - Also considering to use PostgreSQL for its excellent built-in JSON support (helpful to manipulate, indexing...)
